...
class CategoryCollection {
  ...
  static Stream<List<Category>> watchAll() {
    return _repository.watchAll().map((categories) {
  ...
  static Future<bool> prepareDefaultCategories() async {
    final allCategoriesAtNow = await _repository.getAll();
  ...
  List<int> ids() {
    if (containsAll) {
      throw Exception(
          'Because containsAll-flag is true, this object does not have ids.');
    }
