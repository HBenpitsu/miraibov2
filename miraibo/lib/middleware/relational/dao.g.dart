// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'dao.dart';

// ignore_for_file: type=lint
mixin _$PlansMixin on DatabaseAccessor<AppDatabase> {
  Categories get categories => attachedDatabase.categories;
  Currencies get currencies => attachedDatabase.currencies;
  ReceiptLogs get receiptLogs => attachedDatabase.receiptLogs;
  OneshotPlans get oneshotPlans => attachedDatabase.oneshotPlans;
  IntervalPlans get intervalPlans => attachedDatabase.intervalPlans;
  WeeklyPlans get weeklyPlans => attachedDatabase.weeklyPlans;
  MonthlyPlans get monthlyPlans => attachedDatabase.monthlyPlans;
  AnnualPlans get annualPlans => attachedDatabase.annualPlans;
  EstimationSchemes get estimationSchemes => attachedDatabase.estimationSchemes;
  MonitorSchemes get monitorSchemes => attachedDatabase.monitorSchemes;
  MonitorSchemeCategoryLinkers get monitorSchemeCategoryLinkers =>
      attachedDatabase.monitorSchemeCategoryLinkers;
}
